...
class ReportBuilder {
    ...
    addClose(order) {
        this.orderLogs.push('(' + order.toString() + ') closed at ' + order.closePrice + ' for ' + order.closeAmount);
    }
...
class Broker {
    ...
    updateInfo(ticker) {
        ...
        let tryTrigger = (order, price) => {
            if (order.tryTrigger(price, this.asset)) {
                this._asset = this.asset.embrace(order.closeAssetDelta);
                this.reportBuilder.addClose(order);
            }
        };
