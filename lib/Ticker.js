...
class Ticker {
     ...
     *}
     */
    constructor(arrayOrJson) {
        let keys = 'timestamp best_ask ltp best_bid volume volume_by_product'.split(' ');
        if (Array.isArray(arrayOrJson)) {
            expect(arrayOrJson.length).to.be.equal(6);
            this.json = {
                timestamp: arrayOrJson[0],
                best_ask: parseInt(arrayOrJson[1]),
                ltp: parseInt(arrayOrJson[2]),
                best_bid: parseInt(arrayOrJson[3]),
                volume: parseInt(arrayOrJson[4]),
                volume_by_product: parseInt(arrayOrJson[5]),
            };
        } else {
            keys.forEach(key => expect(arrayOrJson).to.have.property(key));
            this.json = arrayOrJson;
        }
    }
